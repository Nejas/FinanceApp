package com.example.financeapp.core.theme

import androidx.compose.ui.graphics.Color
val FinanceBackground = Color(0xFFFEF7FF)
val FinanceNavigationBackground = Color(0xFFF3EDF7)
val FinanceSelectedContainer = Color(0xFFE8DEF8)
val FinancePrimary = Color(0xFF21005D)
val FinanceOnSecondary = Color(0xFF7B5EA7)
val FinanceOnBackground = Color(0xFF1D1B20)
val FinanceMutedText = Color(0x8049454F)
val FinanceDonutLabelText = Color(color = 0xFF49454F)
val FinanceOutline = Color(0xFF49454F)
val FinanceOutlineVariant = Color(0xFFC4C7C5)
val FinanceSurface = Color(0xFFFFFFFF)
val FinanceDatePickerContainer = Color(0xFFECE6F0)
val FinanceDatePickerSelectedDay = Color(0xFF6750A4)
val FinanceTimePickerContainer = Color(0xFFECE6F0)
val FinanceTimePickerField = Color(0xFFE6E0E9)
val FinanceCurrencyCheck = Color(0xFF7B61FF)
val FinancePositive = Color(0xFF2E7D32)
val FinancePendingSyncContainer = Color(0xFFFFE0B2)
val FinanceOnPendingSyncContainer = Color(0xFF4A2800)
val FinanceWarning = Color(0xFFB3261E)
val FinanceWarningContainer = Color(0xFFF9DEDC)
val FinanceOnWarningContainer = Color(0xFF3D2B00)
val FinanceSecurityControlContainer = Color(0xFFF2F2F7)
val FinanceBiometricIconContainer = Color(0x1A7B61FF)

val ColorDarkBackground = Color(0xFF1D1B20)
val ColorDarkSurfaceVariant = Color(0xFF302D36)
val ColorDarkSelectedContainer = Color(0xFF4B3A68)
val ColorDarkOnBackground = Color(0xFFFFF7FF)
val ColorDarkOutline = Color(0xFF6D6670)
val ColorDarkOnWarningContainer = Color(0xFFFFE8A3)
val ColorDarkDanger = Color(0xFFF2B8B5)
val ColorDarkPendingSyncContainer = Color(0xFF5C3B00)
val ColorDarkOnPendingSyncContainer = Color(0xFFFFDDB3)
val ColorDarkSecurityControlContainer = Color(0xFF302D36)

val ChartBlue = Color(0xFF6C8CFF)
val ChartPurple = Color(0xFF9D7BEA)
val ChartPink = Color(0xFFE47AC3)
val ChartGreen = Color(0xFF6BBF8A)
val ChartOrange = Color(0xFFF0A34B)
val AnalyticsChartPurple = Color(0xFFB69DF8)
val AnalyticsChartCyan = Color(0xFF4FD8EB)
val AnalyticsChartPink = Color(0xFFF48FB1)
val AnalyticsChartPalette = listOf(
    AnalyticsChartPurple,
    AnalyticsChartCyan,
    AnalyticsChartPink,
    ChartGreen,
    ChartOrange,
    ChartBlue,
    Color(0xFFE57373),
    Color(0xFFFFD54F),
    Color(0xFF4DB6AC),
    Color(0xFFBA68C8)
)
