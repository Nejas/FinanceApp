package com.example.financeapp.presentation.common.components.icons
...
fun FinanceBarChartIcon(
    ...
    Canvas(modifier = modifier) {
        val strokeWidth = size.minDimension * 1.5f / 24f
        val yBottom = size.height * (20f / 24f)

        drawLine(
            color = color,
            start = Offset(size.width * (6f / 24f), yBottom),
        ...
        drawLine(
            color = color,
            start = Offset(size.width * (9f / 24f), yBottom),
            end = Offset(size.width * (9f / 24f), size.height * (13f / 24f)),
            strokeWidth = strokeWidth,
            cap = StrokeCap.Round
        )
        drawLine(
            color = color,
            start = Offset(size.width * (13f / 24f), yBottom),
            end = Offset(size.width * (13f / 24f), size.height * (9f / 24f)),
            strokeWidth = strokeWidth,
            cap = StrokeCap.Round
        )
        drawLine(
            color = color,
            start = Offset(size.width * (17f / 24f), yBottom),
            end = Offset(size.width * (17f / 24f), size.height * (15f / 24f)),
            strokeWidth = strokeWidth,
            cap = StrokeCap.Round
        )
        drawLine(
            color = color,
            start = Offset(size.width * (21f / 24f), yBottom),
            end = Offset(size.width * (21f / 24f), size.height * (6f / 24f)),
            strokeWidth = strokeWidth,
            cap = StrokeCap.Round
        )
    }
